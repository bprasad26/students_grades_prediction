...
def ci_mean_std_known(array, std, conf_level=95):
    ...
    lcb = np.round(mean - z_star * std_error, 2)
    ucb = np.round(mean + z_star * std_error, 2)
    print(
...
def ci_mean_std_unknown(array, conf_level=95):
    ...
    # calculate the lower and upper confidence bounds
    lcb = np.round(mean - t_star * std_error, 2)
    ucb = np.round(mean + t_star * std_error, 2)
    print(
        "{}% Confidence Interval for Population Mean: ({},{})".format(
            conf_level, lcb, ucb
        )
...
def ci_prop(p, n, conf_level=95):
    ...
    lcb = np.round(p - z_star * std_error, 2)
    ucb = np.round(p + z_star * std_error, 2)
